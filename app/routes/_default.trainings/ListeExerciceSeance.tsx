...
import { SideBarPopover } from "~/ui/shared/SideBarPopover"
          ...
          <li key={`${exercice.nomExercice}-${exerciceItemIdx}`} className="bg-white rounded-lg shadow px-4 pt-4 mb-4">
                  ...
                  <SideBarPopover>
                    <fetcher.Form method="DELETE">
                      ...
                      <button
                        type="submit"
                        name="_action"
                        value="supprimer-exercice"
                        className="relative rounded-lg p-4 hover:bg-gray-50 flex justify-center w-full"
                      >
            ...
            {exercice.series.map((serie, idSerie) => (
              <div key={`${exercice.nomExercice}-${serie.ordre}-${idSerie}`} className="relative flex gap-x-4">
                <div className="relative pb-4">
                  <span
                    className="absolute left-5 bottom-10 -ml-px h-6 w-0.5 bg-main-kauri-lighter z-[1]"
                    aria-hidden="true"
                  />
                  <div className="relative flex items-start space-x-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-xs text-gray-500">
                        <span className="text-gray-900">Série {serie.ordre}</span> ⬝ Reps : {serie.repetitions} ⬝ Repos
                        : {serie.tempsRepos}s ⬝ Poids : {serie.poids}kg
                      </div>
                    </div>
                  </div>
                </div>
